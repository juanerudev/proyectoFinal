...
fun PlanetsListScreen(
    ...
    val planetsState = viewModel.planetsState.value

    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("PLANETAS", fontWeight = FontWeight.Bold) },
                navigationIcon = {
        ...
        when (planetsState) {
            ...
            is NetworkResult.Error -> ErrorMessage(planetsState.message)
            is NetworkResult.Success -> {
                LazyColumn(
                    ...
                    items(planetsState.data) { planet ->
                        Card(
                            ...
                            Column(modifier = Modifier.padding(16.dp)) {
                                ...
                                Row(
                                    ...
                                    Column(modifier = Modifier.weight(1f)) {
                                        DetailText("Clima", planet.climate)
                                        DetailText("Terreno", planet.terrain)
                                    }
                                    Column(modifier = Modifier.weight(1f)) {
                                        DetailText("Población", planet.population)
                                        DetailText("Diámetro", "${planet.diameter} km")
                                    }
                                }
                            }
                        }
